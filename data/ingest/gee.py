...
def main() -> None:
    ...
    base_out = args.out or Path("data/output")
    config = GeeConfig(
        project=args.project,
        chirps_dir=base_out / "chirpsv3",
        s1_dir=base_out / "sentinel1",
    )
    config.chirps_dir.mkdir(parents=True, exist_ok=True)
    ...
    logger.info("Done. %d output file(s) written to %s, %s", len(written), config.chirps_dir, config.s1_dir)
    for p in written:
        logger.info("  %s", p)
