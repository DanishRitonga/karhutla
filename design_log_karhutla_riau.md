...
| ERA5 | Suhu udara, kelembapan, kecepatan angin | Dinamis harian |
...
**Pertanyaan yang dijawab** Berapa performa maksimum yang dapat dicapai sistem operasional yang memakai seluruh informasi tersedia.
...
| --- | --- | --- |
| Dinamis temporal | ERA5, CHIRPS | Channel penuh, bervariasi tiap hari |
...
Komponen berikut secara sengaja dikeluarkan dari kontribusi ilmiah dan ditempatkan sebagai luaran aplikatif atau future work.

**LLM dan RAG** Dikeluarkan dari judul dan dari klaim kontribusi. Menyusun narasi dari angka risiko yang sudah dihitung tidak meningkatkan kemampuan prediksi, sehingga tidak dapat diklaim sebagai novelty. Tetap dapat dibangun sebagai lapisan aplikasi pada dashboard setelah seluruh tahap inti selesai.
...
Fase 5  Dashboard dan LLM, opsional
```
...
**Resolution Trigger** Autokorelasi spasial label selesai dihitung, misalnya melalui variogram atau Moran's I pada label hotspot.

**Resolution Rule**

Konteks spasial yang diberikan kepada model tabular harus menyamai receptive field efektif model spatiotemporal. Radius ring tetangga untuk fitur agregat tabular ditetapkan berdasarkan jangkauan autokorelasi spasial terukur, dan disesuaikan agar setara dengan patch 15 x 15 yang diterima ConvLSTM.
...
Pada operational regime, fitur hotspot historis hanya bersumber dari t-13 hingga t. Tidak boleh ada observasi dari t+1 hingga t+7 yang masuk ke fitur.
...
| Data sources | FIRMS, ERA5, CHIRPS, Sentinel-1, WorldCover, gambut | DECIDED |
...
| LLM | Dikeluarkan dari kontribusi, opsional sebagai dashboard | DECIDED |
